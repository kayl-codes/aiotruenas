"""Generic declarative-mapping engine for normalizing TrueNAS API payloads.

Ported from the ``apiparser.py`` module shared (previously duplicated) by
``truenas_ce``'s Home Assistant integration and HACS custom component. Pure,
HA-independent data transformation: takes a raw ``dict``/``list`` payload as
returned by ``TrueNASClient.call()`` and a declarative field-spec table
(``ApiValueSpec``), and produces a normalized ``dict`` keyed by a chosen id
field. Kept separate from ``domain/state.py`` so the (data-driven) mapping
logic can be unit-tested in isolation from any TrueNAS domain knowledge.
"""
