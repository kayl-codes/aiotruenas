...
`get_alerts()`. This covers the full endpoint set migrated from consumer integrations' own
normalization code — see [MIGRATION_PLAN.md](MIGRATION_PLAN.md) for details and history.

`get_arc()`/`get_ups()`/`get_alerts()` are the exception to the dict-keyed-by-id shape above: they
have no natural object id and instead return a flat dict of scalar/aggregate readings (e.g.
`{"battery_charge": 80.0, ...}` or, for alerts, `{"count": 2, "critical": 1, ...}`).
