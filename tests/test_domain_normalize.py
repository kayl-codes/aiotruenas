"""Unit tests for ``aiotruenas.domain._normalize``.

Ported from ``truenas_ce``'s ``tests/test_apiparser.py`` (the module this file
tests is a verbatim port of that integration's ``apiparser.py``). Pure-function
tests only — no TrueNAS client/I/O involved.
"""
...
from aiotruenas.domain import _normalize


@pytest.fixture
def ap() -> ModuleType:
    """The ``aiotruenas.domain._normalize`` module under test."""
    return _normalize
...
def test_parse_api_convert_human_date_non_string_becomes_none(ap: ModuleType) -> None:
    """A malformed (non-string) certificate date must normalize to None."""
    result = ap.parse_api(
